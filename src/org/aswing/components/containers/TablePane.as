package org.aswing.components.containers
{
	...
	public class TablePane extends JPanel implements LayoutManager
	{
		...
		public function addRow(isExpanded:Boolean = false, components:Array = null):void
		{
			if (rows.length == 0 || rows[rows.length - 1].length != 0)
				rows.push(new Vector.<Cell>); //only add if needed
			if (isExpanded)
			{
				if (expandRow != -1 && rows.length - 1 != expandRow)
					throw new ArgumentError('You can only set one expanded row.');
		...
		public function addRows(numRows:uint = 2):void{
			while (numRows--){
				addRow();
			}
		}
		...
		//public function addCells(..components)
		
		public function addCells( /*gap:int,*/... comps):void
		{
			var textRenderer:Class = DSLabel;
			...
			for (var i:int = 0; i < comps.length; i++)
			{
				...
				if (obj is int)
				{
					comps.splice(i, 1, new JSp(obj));
				}
				if (obj is String)
				{
					//var textElement:* = new textRenderer;
					var textElement:* = new textRenderer;
					textElement.setText(obj);
					comps.splice(i, 1, textElement);
				}
				if (obj is Array)
				{
					//if(
					...
					if (arr[0] is int)
					{
						comps.splice(i, 1, new JSp(arr[0], arr[1]));
					}
					else
					{
						...
						//var jpanel:JPanel = new JPanel(new FlowLayout());
						//jpanel.append
			...
			for each (var c:Component in comps)
				addCell(c);
			//p.append(c);
		...
		{
			var label:DSLabel = new DSLabel(text, null, JLabel.LEFT);
			//label.setForeground(new ASColor(0xFFFFFF, 0.65));
			label.setForeground(new ASColor(0x4a626e, 1));
		...
		public function preferredLayoutSize(target:Container):IntDimension
		{
			...
			var width:int = arraySum(colSizes) + hGap * (colCount - 1);
			var height:int = arraySum(rowHeights) + vGap * (rows.length - 1);
			
			var dim:IntDimension = new IntDimension(width, height);
			return getInsets().getOutsideSize(dim);
		}
		
		private function arraySum(arrayOrVector:*):Number
		{
			var sum:Number = 0;
		...
		public function layoutContainer(target:Container):void
		{
			...
			if (expandCol != -1)
			{
				...
				colSizes[expandCol] = w - arraySum(colSizes) - hGap * (colCount - 1);
			}
			if (expandRow != -1)
			{
				rowHeights[expandRow] = 0;
				rowHeights[expandRow] = h - arraySum(rowHeights) - vGap * (rows.length - 1);
			}
