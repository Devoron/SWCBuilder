...
package devoron.components.multicontainers.timeline.components
{
	 ...
	 * @private
	 */
	public class TimelineScrollBarUI extends BaseComponentUI
	{
		...
		private var scrollTimer:ASTimer;
		private var scrollIncrement:int;
		private var scrollContinueDestination:int;
		private var decoratorDO:DisplayObject;
		
		public function TimelineScrollBarUI()
		{
			scrollBarWidth = 16;
		...
		protected function installComponents():void
		{
			...
			if (thumbDecorator != null)
			{
				...
				{
					decoratorDO = thumbDecorator.getDisplay(scrollbar);
					thumMC.addChild(decoratorDO);
				}
			}
			
			/*var sp:Sprite = new Sprite();
			   sp.graphics.beginFill(0x0080C0);
			   sp.graphics.drawRect(0, 0, 20, 20);
			   sp.graphics.endFill();
			
			   var sp2:Sprite = new Sprite();
			   sp2.graphics.beginFill(0x0080C0);
			   sp2.graphics.drawRect(0, 0, 20, 20);
			...
			//thumMC.addChild(new ColorDecorator(new ASColor(0x0080C0)).getDisplay(
			
			scrollbar.addChild(thumMC);
			//thumMC.addEventListener(MouseEvent.MOUSE_DOWN, __startDragThumb);
			decoratorDO.addEventListener(MouseEvent.MOUSE_DOWN, __startDragThumb);
			...
			//scrollbar.setBackgroundDecorator(new ColorDecorator(new ASColor(0x00FF40)));
		}
		...
		protected function installListeners():void
		{
			...
			scrollTimer = new ASTimer(scrollSpeedThrottle);
		...
		private function __startDragThumb(e:Event):void
		{
			...
			// проверка на то, что он потянут слева
			if ((mx - 20) < tr.x)
			{
				omg = false;
				scrollbar.stage.addEventListener(MouseEvent.MOUSE_MOVE, __onMoveThumb2, false, 0, true);
			}
			// или справа
			else if (mx > (tr.rightBottom().x - 20))
			{
				omg = true;
				scrollbar.stage.addEventListener(MouseEvent.MOUSE_MOVE, __onMoveThumb2, false, 0, true);
			}
			// иначе - перетаскивание метки
			else {
				__startHandleDrag();
			}
		...
		private var omg:Boolean = false;
		
		private function __onMoveThumb2(e:MouseEvent):void
		{
			if (!scrollbar.isEnabled())
			{
				return;
			...
			var mx:int = mp.x;
			var tr:IntRectangle = getThumbRect();
			offset = tr.rightBottom().x - mx;*/
			scrollThumbToCurrentMousePosition2();
			
			
			e.updateAfterEvent();
		
			//gtrace("after " + getThumbRect());
		}
		
		private function scrollThumbToCurrentMousePosition2():void
		{
			//decoratorDO.scaleX += 1.5;
			...
			//gtrace("thumbR " + thumbR);
			gtrace("thumbR.x " + thumbR.rightBottom().x + " mx " + mx);
			
			//thumbR.x
			/*if (thumbPos < thumbR.x) {
			   setThumbRect(thumbPos, thumbR.y, thumbR.width +(thumbR.x - thumbPos ) , thumbR.height);
			   }
			   else if (thumbPos > thumbR.x) {
			   setThumbRect(thumbPos, thumbR.y, thumbR.width +(thumbPos - thumbR.x) , thumbR.height);
			   }*/
			
			// в направлении направо
			if (omg)
			{
				//gtrace("thumbPos " + thumbPos);
				//gtrace("thumbR " + thumbR);
				setThumbRect(thumbR.x, thumbR.y, thumbR.width + (mx - thumbR.rightBottom().x), thumbR.height);
			}
			// в направлении влево
			else
			{
		...
		{
			scrollbar.stage.removeEventListener(MouseEvent.MOUSE_MOVE, __onMoveThumb2);
		}
		...
		private function paintAndLocateThumb(b:IntRectangle):void
		{
			if (!scrollbar.isEnabled())
			{
				if (isVertical())
				{
					...
					{
						gtrace("Logic Wrong : Scrollbar is not enabled, but its children enabled ");
					}
				}
			...
			{
				if (range < 0)
					gtrace("Logic Wrong : Scrollbar range = " + range + ", max=" + scrollbar.getMaximum() + ", min=" + min);
				thumMC.visible = false;
				return;
			}
			...
			{
				//gtrace("The visible range is so short can't view thumb now!");
				thumMC.visible = false;
				return;
			}
			...
			if (isVertical())
			{
				setThumbRect(b.x, thumbPos + b.y + decrButton.getHeight(), scrollBarWidth, thumbLength);
			}
			else
			{
				setThumbRect(thumbPos + b.x + decrButton.getWidth(), b.y, thumbLength, scrollBarWidth);
			}
		}
