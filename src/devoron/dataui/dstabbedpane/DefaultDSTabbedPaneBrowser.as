package devoron.components.data.dstabbedpane
{
	...
	import devoron.light.LightData;
	import devoron.mesh.MeshData;
	import devoron.objectcontainer3D.ObjectContainer3DData;
	import devoron.utils.ArrayNamesHelper;
	import devoron.values.models.TreeModelValueComponent;
	import devoron.view.ViewData;
	 ...
	 * @author ...
	 */
	public class DefaultDSTabbedPaneBrowser extends StudioModul /*implements IPropertiesPanelComponent*/
	{
		...
		protected var contentTree:ContentTree;
		
		protected var contentTabbedPane:JTabbedPane;
		
		public var contentTreeScP:JScrollPane;
		...
		public function DefaultDSTabbedPaneBrowser(modulName:String = "", modulIcon:Class = null, modulVersion:String = "")
		{
			...
			setTopControlsPanel(getTopControlsPanel());
			
			//createContentTree();
			createContentTabbedPane();
			subModulForm.addLeftHoldRow(0, contentTreeScP);
			...
			var treeModelValueComponent:TreeModelValueComponent = new TreeModelValueComponent(contentTree);
			...
			new StudioMediator("Find and replace", setFindAndReplaceTool);
		}
		...
		public function getContentTree():JTree
		{
			return contentTree;
		}
		...
		public function setRoot():void
		{
			...
			theroot = new ContentTreeNode(dataStructur, editor.getModulIcon());
			contentTreeModel.setRoot(theroot);
			contentTree.revalidate();
			//theroot = new ContentTreeNode(dataStructur, new EmptyIcon());
			//contentTreeModel = new DefaultTreeModel(theroot);
		}
		...
		public function setModuls(moduls:Array):void
		{
			...
			for (var i:int = 0; i < moduls.length; i++)
			{
				...
				else if (modul is OperationBox)
				{
					...
					contentTree.setSelectionPath(new TreePath(dsNode.getPath()));
		...
		public function addDataContainerForm(dcf:DataContainerForm, dataName:String, dataIcon:Icon):DefaultMutableTreeNode
		{
			...
			contentTree.setSelectionPath(new TreePath(node.getPath()));
		...
		public function createContentTabbedPane():void
		{
			contentTabbedPane = new JTabbedPane();
			//contentTabbedPane.set
			contentTabbedPane.setPreferredWidth(267);
			//contentTabbedPane.setTabPlacement(JTabbedPane.BOTTOM);
			var advProp:ASFontAdvProperties = new ASFontAdvProperties(true, "advanced", "pixel");
			var font:ASFont = new ASFont("Consolas", 12, false, false, false, advProp);
			contentTabbedPane.setUI(new StaionTabbedPaneUI());
			contentTabbedPane.setFont(font);
			
			contentTreeScP = new JScrollPane(contentTabbedPane);
			//contentTreeScP.setPreferredSize(new IntDimension(267, 280));
			contentTreeScP.setPreferredSize(new IntDimension(267, 400));
			contentTreeScP.setVerticalScrollBarPolicy(JScrollPane.SCROLLBAR_AS_NEEDED);
			contentTreeScP.setHorizontalScrollBarPolicy(JScrollPane.SCROLLBAR_AS_NEEDED);
			//contentTreeScP.setHorizontalScrollBarPolicy(JScrollPane.SCROLLBAR_ALWAYS);
			contentTreeScP.buttonMode = true;
		}
		
		protected function createContentTree():void
		{
			
			//var editor:BaseEditor = (moduls[selId] as BaseEditor);
			...
			var editor:BaseDataStructurProcessor = datasAndEditors.get(dataClass);
			//theroot = new ContentTreeNode(dataStructur, editor.getModulIcon());
			theroot = new ContentTreeNode(dataStructur, new EmptyIcon(20, 20));
			contentTreeModel = new DefaultTreeModel(theroot);
			//contentTreeModel = new DefaultTreeModel();
			
			contentTree = new ContentTree(contentTreeModel);
			contentTree.addActionListener(contentTreeChange);
			contentTreeSelectionModel = contentTree.getSelectionModel();
			contentTree.setPreferredWidth(267);
			contentTree.addSelectionListener(contentTreeSelectionHandler);
			contentTree.setCellFactory(new GeneralTreeCellFactory(ContentTreeCellRenderer));
			
			contentTreeScP = new JScrollPane(contentTree);
			//contentTreeScP.setPreferredSize(new IntDimension(267, 280));
			contentTreeScP.setPreferredSize(new IntDimension(267, 400));
			contentTreeScP.setVerticalScrollBarPolicy(JScrollPane.SCROLLBAR_ALWAYS);
			contentTreeScP.setHorizontalScrollBarPolicy(JScrollPane.SCROLLBAR_AS_NEEDED);
			//contentTreeScP.setHorizontalScrollBarPolicy(JScrollPane.SCROLLBAR_ALWAYS);
			contentTreeScP.buttonMode = true;
		
			//contentTree.setCellFactory(
		}
		...
		override public function setOwnerContainer(ownerContainer:Container):void
		{
			...
			ownerContainer.append(modulForm, BorderLayout.EAST);
		}
		...
		public function copySelectedNode():void
		{
			...
			if (true)
			{
				...
				if (dataStructur)
				{
					...
					contentTree.setSelectionPath(new TreePath(newNode.getPath()));
				}
		...
		public function addDataStructur(dataStructurClass:Class, dataName:String, dataIcon:Icon):void
		{
			...
			form.addLeftHoldRow(0, lb);
				...
				contentTabbedPane.appendTab(form, dataStructur.dataType /*, dataContainer.icon*/);
